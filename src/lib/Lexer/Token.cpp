...
Token::Token()
    : value(tok::TokenValue::FILE_EOF), loc(0, 0, std::string("")), intValue(0),
      realValue(0), strValue(""), lexem("") {}

// identifier and keyword
Token::Token(TokenValue tv, const TokenLocation &location, std::string name)
    : value(tv), loc(location), intValue(0), realValue(0), strValue(name),
      lexem(name) {}

// literal
// Token::Token(TokenValue tv, const TokenLocation& location, const std::string&
// strValue, std::string name) : 	value(tok::TokenValue::FILE_EOF), Length(0),
...
Token::Token(TokenValue tv, const TokenLocation &location, long intvalue,
             std::string name)
    : value(tv), loc(location), intValue(intvalue), realValue(0), strValue(""),
      lexem(name) {}
// real
Token::Token(TokenValue tv, const TokenLocation &location, double realvalue,
             std::string name)
    : value(tv), loc(location), intValue(0), realValue(realvalue), strValue(""),
      lexem(name) {}