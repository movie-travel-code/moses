...
namespace compiler
{
	namespace parse
	{
		...
		{
			CurLoc = getTokenLocation();
			makeToken(tok::TokenValue::FILE_EOF, CurLoc, std::string("FILE_EOF"));
			input.close();
		}
		...
		void Scanner::handleNumberState()
		{
			...
			if (!errorFlag)
			{
				if (isFloat)
				{
					makeToken(tok::TokenValue::REAL_LITERAL, CurLoc, std::stod(buffer), buffer);
				}
				else
				{
					makeToken(tok::TokenValue::INTEGER_LITERAL, CurLoc, std::stol(buffer, 0, numberBase), buffer);
				}
			}
			else
			{
		...
		void Scanner::handleStringState()
		{
			...
			if (buffer.length() == 1)
			{
				makeToken(tok::TokenValue::CHAR_LITERAL, CurLoc, static_cast<long>(buffer.at(0)), buffer);
			}
			else
			{
				makeToken(tok::TokenValue::STRING_LITERAL, CurLoc, buffer);
			}
		}
		...
		void Scanner::handleIdentifierState()
		{
			...
			tok::TokenValue tokenValue = table.isKeyword(buffer);
			if (tokenValue == compiler::tok::TokenValue::UNKNOWN)
			{
				// 不是关键字，说明tokenValue是 identifier
				tokenValue = compiler::tok::TokenValue::IDENTIFIER;
			}
			// 返回一个创建好的token
		...
		void Scanner::handleOperationState()
		{
			...
			if (tokenKind == compiler::tok::TokenValue::UNKNOWN)
			{
				// 不是任意字符，则后退一步
				reduceBuffer();
